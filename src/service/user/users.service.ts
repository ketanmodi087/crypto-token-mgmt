...
export class UserService {
	...
	async updateUser(userId: string, body: UpdateUserProfileDto, file: Express.Multer.File = null, bucketName: string = null): Promise<IUser> {
		let key = null;
		if(!!file){
		...
		const existingUser = await this.userModel.findByIdAndUpdate(userId, file? {...body,
			profile: key} : {...body});
		if (!existingUser) {
	...
	async updateAccountSettings(userId: string, body: UpdateAccountSettingsDto): Promise<IUser> {
		const existingUser = await this.userModel.findByIdAndUpdate(userId, {...body});
		if (!existingUser) {
			throw new NotFoundException(`User #${userId} not found`);
		}
		return existingUser;
	...
	async updateKyc(userId: string, UpdateKycDto: UpdateKycDataDto, passport_url: any = null, user_photo_url: any = null): Promise<any> {
		...
		if(!!user_photo_url && !!user_photo_url.buffer){
			...
			const s3 = this.configService.get('s3');
			const bucketName = this.configService.get('aws_s3_bucket_name');
			user_photo_url_key = new Date().valueOf() + "_" + user_photo_url.originalname;
	...
	async getUser(userId: string): Promise<any> {
		const existingUser = await this.userModel.findById(userId).select('-_id -__v -nonce').exec();
		if (!existingUser) {
			throw new NotFoundException(`User #${userId} not found`);
		}
		return existingUser;
	...
	async getFindbyAddress(address: string): Promise<any> {
		const existingUser = await this.userModel.findOne({wallet_address:address}).exec();
		return existingUser;
	}
	...
	async getOnlyUserBioByAddress(address: string): Promise<any> {
		const existingUser = await this.userModel.findOne({wallet_address:address}, ).select('-_id -nonce -__v').exec();
		if (!existingUser) {
			throw new NotFoundException(`Address #${address} not found`);
		}
		return existingUser;
